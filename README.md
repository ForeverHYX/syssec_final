...
默认 benchmark 现在把 13 个合成 oracle APK 和 12 个外部现成 APK 合并评分，共 25 个样本。评分表只包含当前环境可安装、可运行且 25/25 样本都有输出的比较对象：HardenInspector、APKiD、Androguard DEX baseline 和 ZIP Strings baseline。DroidLysis/MobSF 不进入默认评分表，避免把缺少外部分析管线造成的不可用结果记成 0 分。
...
| --- | --- |
| 自动化测试 | `31 passed` |
| 合并评分数据集 | 13 个 synthetic APK + 12 个外部 APK，四个默认评分工具均为 25/25 coverage |
| Combined benchmark Micro F1 | HardenInspector 0.866；APKiD 0.341；Androguard DEX 0.593；ZIP Strings 0.794 |
| 外部现成 APK 语料 | 已纳入评分；单独统计中四个工具仍为 12/12 coverage |
| 外部语料 HardenInspector 分布 | Any 9/12；packer=4；obfuscation=6；environment=3；native=0；F-Droid 样本无 finding |
...
| 环境检测 | emulator system properties、debugger probe、Native anti-debug 符号、Frida/Xposed/Substrate/process maps |
| Native | `JNI_OnLoad` 字符串/ELF 符号等 Native 入口证据 |

...
- 规则覆盖课程目标中的加壳、代码混淆、环境感知检测，并补充 Native 入口证据。
...
默认 benchmark 现在把 22 个合成 oracle APK 和 12 个外部现成 APK 合并评分，共 34 个样本。评分表只包含当前环境可安装、可运行且 34/34 样本都有输出的比较对象：HardenInspector、APKiD、Androguard DEX baseline 和 ZIP Strings baseline。DroidLysis/MobSF 不进入默认评分表，避免把缺少外部分析管线造成的不可用结果记成 0 分。
...
| --- | --- |
| 自动化测试 | `69 passed` |
| 合并评分数据集 | 22 个 synthetic APK + 12 个外部 APK，四个默认评分工具均为 34/34 coverage |
| Combined benchmark Micro F1 | HardenInspector 1.000；APKiD 0.314；Androguard DEX 0.609；ZIP Strings 0.759 |
...
| Web Demo | 首屏展示 Exhibit Map、Evidence Chain、Dataset Story、34 个评分 APK、69 个测试和上传扫描 |
...
浏览器访问 `http://127.0.0.1:8000/`。页面首屏先展示项目动机、证据链、Synthetic Oracle + External APK Corpus 的数据集结构、34 个评分 APK、69 个回归测试和 HardenInspector Micro F1。随后可以选择 clean baseline、综合加固样本、ADB/developer-settings、installer-source、Java Debug API、signature integrity、root artifact、Native/IMEI 专项样本和外部 APK，也可以上传本地 APK 临时扫描，现场查看 summary、finding evidence 和 benchmark 对比指标。
...
| 环境检测 | emulator system properties、emulator file artifacts、IMEI/device-id probe、ADB/developer-settings probe、installer-source/sideload probe、Java Debug API、signature integrity/self-check、root/Magisk/su artifact probe、debugger probe、Native anti-debug 符号、Frida/Xposed/Substrate/process maps |

...
中期报告提出的完整框架包括解析层、特征层、决策层和可选动态验证层。期末实现保留前三层，动态验证层明确列为当前交付范围之外。
...
| `src/hardeninspector/native.py` | 解析 ELF `.dynsym`/`.symtab` 与字符串表，提取 Native 符号证据 |

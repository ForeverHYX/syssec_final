...
| 构造报告中提到的数据集 | `datasets/hardeninspector_eval_v1/` 包含 18 个 APK、`labels.json` 和 per-sample reports |
...
| 公平 benchmark 对比 | 默认评分表包含 18 个合成 APK + 12 个外部 APK；HardenInspector、APKiD、Androguard DEX 和 ZIP Strings 均为 30/30 coverage；DroidLysis/MobSF 不进入评分表 |
...
| 现场可视化展示 | `make demo-web` 启动本地 Web demo，首屏展示 Exhibit Map、Evidence Chain、Dataset Story、APK 拆解图、30 个评分 APK、50 个测试和 HardenInspector Micro F1；页面可扫描 curated APK 样本、上传本地 APK，并展示 evidence 与 benchmark 指标 |
...
| 维护 GitHub repo `syssec_final` | 远端为 `git@github.com:ForeverHYX/syssec_final.git`，GitHub URL 为 `https://github.com/ForeverHYX/syssec_final`；最新提交已推送到 `origin/main` |
...
结果：fresh venv benchmark 生成 `/tmp/hardeninspector-benchmark-check/benchmark_results.json`；HardenInspector、APKiD、Androguard DEX、ZIP Strings 均为 30/30 coverage，Micro F1 分别为 1.000、0.340、0.533、0.714。
...
结果：重新生成 `reports/benchmark/`，合成 + 外部共 30 个评分样本；HardenInspector Micro F1 为 1.000，APKiD 为 0.340，Androguard DEX 为 0.533，ZIP Strings 为 0.714；所有评分工具 coverage 都是 30/30。

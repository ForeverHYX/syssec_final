...
| `installer_source_probe` | installer-source environment sample | `getInstallSourceInfo` / `getInstallerPackageName` + installer/sideload indicators | 验证安装来源、未知来源或 ADB 安装探测识别 |
...
外部 APK 已补充粗粒度 `expected_categories` 和 `label_basis`，随 `make benchmark` 进入 34 样本合并评分；`make external-corpus` 仍单独输出覆盖率和 finding 分布统计。详见 `docs/external_corpus.md`。

...
- `environment`：emulator/debugger/instrumentation/telephony/signature integrity/native anti-debug 证据；
...
HardenInspector Micro F1 = 1.000 是在当前 30 个合并评分样本和四类粗标签定义下得到的结果，不等价于宣称覆盖所有真实 APK。

这个分数有三层防守：

- 18 个合成 oracle APK 覆盖课程目标内的 packer、obfuscation、environment、native 关键信号；

...
- `environment`：emulator/debugger/ADB developer settings/installer source/Java Debug API/instrumentation/telephony/signature integrity/root artifacts/native anti-debug 证据；
...
- 22 个合成 oracle APK 覆盖课程目标内的 packer、obfuscation、environment、native 关键信号；
...
预置样本适合稳定展示，上传扫描适合现场验证工具不是写死样本结果。Web demo 的 `扫描上传文件` 使用同一个 `scan_apk` pipeline：

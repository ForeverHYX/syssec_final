...
因为它形成了完整闭环：检测器源码、CLI、Web demo、上传扫描、22 个合成 oracle APK、12 个外部现成 APK、四个比较对象、Precision/Recall/F1 指标、71 个自动化测试、最终报告和 Beamer 汇报稿。每条 finding 都能回溯到 APK 内的 evidence。
...
先讲项目动机、证据链、数据集结构、34 个评分 APK、71 个测试和 HardenInspector Micro F1。然后展示样本列表、上传 APK、扫描结果 summary、finding evidence 和 benchmark 表。
...
71 个自动化测试覆盖项目环境、APK/AXML 解析、DEX 解析、Native ELF 符号解析、规则命中、CLI、dataset 构造、benchmark、Web demo 接口和最终交付材料一致性。
...
先跑 `.venv/bin/python -m pytest -q` 证明 71 个测试通过；再跑 `make benchmark` 证明 34 个样本和四个工具的指标可生成；最后扫一个 APK 展示 JSON evidence。

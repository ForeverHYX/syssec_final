...
| 构造小规模测试集 | `datasets/hardeninspector_eval_v1/` 包含 10 个 APK、labels 和报告 |
| 与开源实现对比并给出统计数据 | `reports/benchmark/benchmark_results.json`、`benchmark_metrics.csv`、`benchmark_summary.md`；默认评分工具均为 10/10 样本可运行，不再把 DroidLysis 不可用环境记为 0 分 |

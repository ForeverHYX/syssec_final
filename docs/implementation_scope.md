...
- 环境检测规则：模拟器 system properties、emulator file artifacts、IMEI/device-id probe、signature integrity/self-check、debugger probe、Frida/Xposed/Substrate/process maps。
...
- 数据集包含 18 个 APK：F-Droid 风格基线、自写环境检测、R8 风格短标识符、Obfuscapk 风格反射/动态加载、两类加壳 stub/payload、Native JNI bridge、Frida/Xposed 探测、reflection-only dispatch、控制流密度样本、高熵 payload-only 样本、Native ptrace/loader 符号样本、Class.forName 反射样本、emulator file artifacts 样本、IMEI/device-id probe 样本、JNI `Java_*` 导出符号样本、signature integrity/self-check 样本、综合展示样本。

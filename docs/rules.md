...
### `environment.adb_settings_probe`

检测应用是否读取 ADB 或开发者选项相关系统设置。

当前信号要求同时出现 Settings 类名证据和具体设置键：
...
单独出现 `adb`、帮助文本或 URL 不会触发该规则；规则需要看到 Android Settings API 与 ADB/developer-settings key 的组合证据。

### `environment.installer_source_probe`

检测应用是否读取安装来源、安装器包名或侧载来源相关信息。

当前规则要求同时出现安装来源 API 证据和具体来源/侧载指示：
...
普通读取 `versionName`、包名或 `getPackageInfo` 的元数据查询不会单独触发该规则；规则需要看到 installer-source API 与安装来源值的组合证据。
...
Java 层 Debug API 证据要求出现 `Landroid/os/Debug;` / `android.os.Debug` 这类类名证据，并同时出现 `isDebuggerConnected` 或 `waitingForDebugger` 这类具体调试器状态 API。普通 debug 日志、`debuggable` 字符串或业务方法名不会单独触发该规则。

...
- Verification checkpoint: full pytest passed, `git diff --check` passed, and `make slides` compiled successfully. The local Web demo smoke test required port-binding escalation and proxy-free `curl`; HTML, `/api/samples`, `/api/scan`, `/api/metrics`, and `/assets/apk-cutaway.png` returned the expected content.

## 2026-05-28 Web Demo Protocol Polish and Slide Demo Narrative

- Started Phase 26 after the Web demo smoke test revealed that `GET /assets/apk-cutaway.png` worked but `HEAD /assets/apk-cutaway.png` returned the default `501 Unsupported method`.
- TDD RED: added `test_demo_asset_head_request_returns_png_headers_without_body`; after fixing the fake socket test helper, it failed on the current `501` response.
- TDD RED: extended `tests/test_final_artifacts.py` to require a dedicated `现场 Web Demo` slide with Exhibit Map, Evidence Chain, Dataset Story, Synthetic Oracle, External APK Corpus, and Scan Upload. The targeted slide test failed because the Beamer deck only mentioned `make demo-web` in the environment slide.

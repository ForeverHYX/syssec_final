...
| 环境检测 | emulator properties、debugger probe、Native anti-debug 符号、Frida/Xposed/process maps |
| Native | `JNI_OnLoad` 字符串/ELF 符号 |
...
外部语料已补充粗粒度 `expected_categories` 和 `label_basis`，随合成数据集一起进入 25 样本合并评分；同时保留单独的覆盖率、finding 分布和普通真实 APK 误报观察。
...
本轮已把原先的“opcode/控制流轻量统计”和“Native 仅字符串扫描”两个短板推进为已实现能力：`dex.py` 输出 opcode profile，`rules.py` 生成 `obfuscation.control_flow_density` finding；`native.py` 解析 ELF 符号表，规则生成 `environment.native_debugger_symbol` 与 `packer.native_dynamic_loader` finding。数据集中新增 `control_flow_flattening.apk`、`high_entropy_payload_only.apk` 和 `native_ptrace_loader.apk` 用于验证。
...
验证结果：当前环境中 31 个测试通过，CLI 可用；四个默认 benchmark 工具均为 25/25 combined scoring coverage，单独外部统计为 12/12 coverage。

| 检查项 | 结果 |
| --- | --- |
| 自动化测试 | 31 个测试通过 |
| 合成数据集 | 13 个 APK，可通过 `make dataset` 复现 |
| Combined benchmark | 13 个 synthetic APK + 12 个外部 APK；四个工具均为 25/25 coverage |
| External corpus | DroidBench/F-Droid/PIVAA 共 12 个现成 APK；单独统计四个工具均为 12/12 coverage |
| Slides | `make slides` 编译为 20 页，构建产物由 `.gitignore` 忽略 |

...
外部语料没有 HardenInspector 四类 hardening 标签，因此不计算 Precision/Recall/F1，只统计工具覆盖率、finding 分布和普通真实 APK 上的误报观察。
...
外部样本用于扩大解析覆盖范围。它们不是 hardening oracle，因此这里只看能否跑完、哪些类别被报告，以及不同工具在真实/公开 APK 上的输出差异。调试中发现 `control_flow_density` 初始阈值对 F-Droid 普通 APK 过敏，已收紧规则；当前 `fdroid_editor` 无 finding。
...
- 在 12 个外部现成 APK 上，HardenInspector 和三个 comparator 均能完成扫描，外部统计单独报告覆盖与分布；
...
验证结果：fresh venv 中 27 个测试通过，CLI 可用；四个默认 benchmark 工具均为 11/11 synthetic coverage 和 12/12 external coverage。
...
Beamer 已使用 `make slides` 编译检查。汇报稿包含规则/benchmark 表格、TikZ 总体架构图、控制流 opcode profile 示意、数据集覆盖矩阵、外部 APK 语料统计、Micro F1 可视化图，以及一张用于说明 APK 拆解对象的生成式示意图；生成的 PDF 和 LaTeX 辅助文件已加入 `.gitignore`。

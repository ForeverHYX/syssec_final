...
验证结果：当前环境中 54 个测试通过，CLI 可用；四个默认 benchmark 工具均为 31/31 combined scoring coverage，单独外部统计为 12/12 coverage。`make demo-web` 可启动本地网页展示，默认监听 `http://127.0.0.1:8000/`，支持预置样本和本地 APK 上传扫描。
...
| 本地 Web Demo | `src/hardeninspector/demo_web.py`、`docs/demo_web.md`、`make demo-web`；首屏包含 Exhibit Map、Evidence Chain、Dataset Story、APK 拆解图、31 个评分 APK、54 个测试和上传扫描 |
| 中文文档 | `docs/` |

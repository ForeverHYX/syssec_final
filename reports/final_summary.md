...
| 环境检测 | emulator properties、emulator file artifacts、IMEI/device-id probe、ADB/developer-settings probe、Java Debug API、signature integrity/self-check、root/Magisk/su artifact probe、debugger probe、Native anti-debug 符号、Frida/Xposed/process maps |
...
验证结果：当前环境中 63 个测试通过，CLI 可用；四个默认 benchmark 工具均为 33/33 combined scoring coverage，单独外部统计为 12/12 coverage。`make demo-web` 可启动本地网页展示，默认监听 `http://127.0.0.1:8000/`，支持预置样本和本地 APK 上传扫描。

| 检查项 | 结果 |
| --- | --- |
| 自动化测试 | 63 个测试通过 |
| 合成数据集 | 21 个 APK，可通过 `make dataset` 复现 |
| Combined benchmark | 21 个 synthetic APK + 12 个外部 APK；四个工具均为 33/33 coverage |
...
| 本地 Web Demo | `src/hardeninspector/demo_web.py`、`docs/demo_web.md`、`make demo-web`；首屏包含 Exhibit Map、Evidence Chain、Dataset Story、APK 拆解图、33 个评分 APK、63 个测试和上传扫描 |
| 中文文档 | `docs/` |

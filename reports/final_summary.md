...
外部语料已补充粗粒度 `expected_categories` 和 `label_basis`，随合成数据集一起进入 23 样本合并评分；同时保留单独的覆盖率、finding 分布和普通真实 APK 误报观察。
...
验证结果：fresh venv 中 28 个测试通过，CLI 可用；四个默认 benchmark 工具均为 23/23 combined scoring coverage，单独外部统计为 12/12 coverage。

| 检查项 | 结果 |
| --- | --- |
| 自动化测试 | 28 个测试通过 |
| 合成数据集 | 11 个 APK，可通过 `make dataset` 复现 |
| Combined benchmark | 11 个 synthetic APK + 12 个外部 APK；四个工具均为 23/23 coverage |
...
Beamer 已使用 `make slides` 编译检查，当前 PDF 为 20 页。汇报稿包含规则/benchmark 表格、TikZ 总体架构图、控制流 opcode profile 示意、数据集覆盖矩阵、外部 APK 语料统计、测试结果复核、Micro F1 可视化图，以及一张用于说明 APK 拆解对象的生成式示意图；生成的 PDF 和 LaTeX 辅助文件已加入 `.gitignore`。

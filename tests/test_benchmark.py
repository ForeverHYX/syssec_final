...
def test_hardeninspector_benchmark_reaches_dataset_oracle(tmp_path):
    ...
    assert result["coverage"]["samples_total"] == 18
    assert result["coverage"]["samples_with_results"] == 18
...
def test_run_benchmark_can_score_external_corpus_samples(tmp_path):
    ...
    assert tool_result["coverage"] == {"samples_total": 19, "samples_with_results": 19}

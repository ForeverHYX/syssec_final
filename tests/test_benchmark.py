...
def test_hardeninspector_benchmark_reaches_dataset_oracle(tmp_path):
    ...
    assert result["coverage"]["samples_total"] == 17
    assert result["coverage"]["samples_with_results"] == 17
...
def test_run_benchmark_can_score_external_corpus_samples(tmp_path):
    ...
    assert tool_result["coverage"] == {"samples_total": 18, "samples_with_results": 18}

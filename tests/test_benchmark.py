...
def test_hardeninspector_benchmark_reaches_dataset_oracle(tmp_path):
    ...
    assert result["coverage"]["samples_total"] == 22
    assert result["coverage"]["samples_with_results"] == 22
...
def test_zip_string_baseline_maps_installer_source_to_environment(tmp_path):
    dataset_dir = tmp_path / "dataset"
    build_dataset(dataset_dir)
    dataset = load_dataset(dataset_dir)
    ...
    assert by_id["installer_source_probe"]["predicted_categories"] == ["environment"]
...
def test_run_benchmark_can_score_external_corpus_samples(tmp_path):
    ...
    assert tool_result["coverage"] == {"samples_total": 23, "samples_with_results": 23}
